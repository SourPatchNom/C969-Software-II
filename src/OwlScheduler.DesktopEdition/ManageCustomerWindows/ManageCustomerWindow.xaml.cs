...
namespace OwlScheduler.DesktopEdition.ManageCustomerWindows
{
    public partial class ManageCustomerWindow : Window
    {
        ...
        public ManageCustomerWindow()
        {
            ...
            ComboAccountPicker.ItemsSource = Library.OwlSchedule.OwlScheduler.Instance.CustomerDataModel.Customers;
        ...
        {
            if (Library.OwlSchedule.OwlScheduler.Instance.CustomerDataModel.Customers.Any(x => x == ComboAccountPicker.SelectedItem as Customer))
            {
                PopulateAccountInfo();
                CheckIfFormComplete();
                return;
        ...
        private void PopulateAccountInfo()
        {
            ...
            ComboAddressPicker.SelectedItem = Library.OwlSchedule.OwlScheduler.Instance.CustomerDataModel.Addresses.First(x => x.AddressId == customer.CustomerAddress); 
            UpdateAddressFields(Library.OwlSchedule.OwlScheduler.Instance.CustomerDataModel.Addresses.First(x => x.AddressId == customer.CustomerAddress));
        }
        ...
        private void UpdateAddressFields(Address address)
        {
            ...
            var city = Library.OwlSchedule.OwlScheduler.Instance.CustomerDataModel.Cities.First(x => x.CityId == address.CityId);
            var country = Library.OwlSchedule.OwlScheduler.Instance.CustomerDataModel.Countries.First(x => x.CountryId == city.Country);
